...
1.  **Claude Codeでプロジェクトを開く:**
    *   Claude Codeを起動し、「File」>「Open Folder」から`my_first_app`フォルダを選択します。

2.  **実行方法:**
    *   Claude Codeには、ターミナルが統合されているので、**内蔵ターミナルで`flutter run`コマンドを実行**するのが最もシンプルです。
...
*   **エディタは自由に選択**: Claude Code、Android Studio、VS Codeなど、好きなエディタでコードを編集できます。
