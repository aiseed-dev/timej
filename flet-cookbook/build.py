#!/usr/bin/env python3
"""docs/ のMarkdownから AI x Flet Cookbook の静的HTMLを生成する。

隣の cookbook/（AI x Flutter Cookbook）と同じ仕組み・同じ web/ テンプレート。
...
SITE_NAME = "AI x Flet Cookbook"

# セクション（表示順）
SECTIONS = [
    ("01_the_kitchen", "1. 台所の準備"),
    ("02_core_concepts", "2. Fletの重要概念"),
    ("03_recipes", "3. 実戦レシピ"),
    ("04_troubleshooting", "4. 救急箱"),
]

# サイドバー表示用にH1タイトルを短くする
_TITLE_PREFIX = re.compile(
    r"^(レシピ|重要概念|救急箱|トラブルシュート)#([\d]+-[\d]+):\s*"
)
...
def main():
    ...
    print(f"OK: {len(pages)}ページを site/ に生成しました")
