...
- **認証(Entra ID)が閉じ込めの本丸** — 「誰であるか」の確認そのもの、
  つまり文書・メール・端末・業務アプリすべての入口を外部クラウドに預ける形になります。
  ローカルの代替は提供されず、新しいシステムを足すときも Entra ID への統合が前提。
  認証を握られているため、個々の部品だけを別の道具に替えることができません
