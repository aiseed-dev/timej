...
CMS を使う組織では、CMS 本体で扱えない業務が「仮の手段」へ押し出されがちです。
申込は後付けのフォームプラグイン、依頼や予約は担当PCの**その場しのぎの Excel**・紙・メール、
内容の編集は「ログインして直す」だけで、承認も版管理もない——
閉じた世界が生む、業務の分断と属人化の現場の姿です。

本キットは、この仮の回避策を一つずつ本格の形に引き上げます。

| 仮の手段 | 本格の形 |
|---|---|
| ログインして直す(履歴なし) | 内容を **Markdown+git** で版管理・共同編集・承認 |
| 後付けフォーム・代表メール | **様式(xlsx)を唯一の定義**とする機械可読な受付経路 |
| その場しのぎのExcel・紙 | 認証・DBを備えた業務アプリ(必要な業務だけ) |

狙いはコスト削減以上に、**所有・統合・監査できる形にすること**です。
ただし全業務を無条件に本格化するのではなく、業務ごとに要否で水準を選びます。
