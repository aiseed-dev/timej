...
<!-- description: 組織のITを開いた土台へ移すためのツール一式。三つの閉じた世界の問題、AIによる接続、amigのパイプライン、様式+メール受付の問い合わせ設計を詳しく説明します。 -->
...
**ソフトウェア開発編の実践そのもの**です。**仕様（定義）を一枚に集め、そこから
コードとデータを導き、AIがコードを書く**——この作り方で、キット自身が作られています。
仕様を直せば実装が追随するので、会社ごとの運用差は個別実装ではなく**定義の差し替え**で
吸収します（憶測の拡張は作らない）。個人の道具（[工房](../kobo/)・[文庫](../bunko/)）と
同じ原則——開いた標準・構造化・テキスト主軸・定義は一枚——を、組織のITという最も大きな
対象に適用する実践であり、作法が個人から組織へ地続きに広がることの実証です。
...
三つに共通するのは、**データと業務の形を道具の側が決めてしまう**こと。
そして、その世界の境界を埋めているのが**人間の手作業**だということです。
...
なお、WordPressを当面使い続けながら**問い合わせデータだけを先に引き取る**緊急措置は
[FormRescue](../formrescue/) にまとめています。フォームは素のHTML+Turnstileになるため、
...
「AIをどう使うか」から始まるこの話は、[自宅AIサーバーのページ](../homeserver/) にまとめています。
